    ...
    for cv in range(ncvs):
        ...
        for cond1 in np.arange(nrules,dtype=int):
            ...
            for cond2 in np.arange(nrules,dtype=int):
                ...
                if cond1==cond2:
                    corr.append(np.arctanh(stats.spearmanr(predicted_miniblock,actualprototype)[0]))
                else:
                    err.append(np.arctanh(stats.spearmanr(predicted_miniblock,actualprototype)[0]))
