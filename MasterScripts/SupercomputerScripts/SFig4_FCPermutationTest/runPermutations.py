...
def runPermutations(cycle):
    ...
    for nPermutation in permutations:
        ...
        inputs = []
        for subj in subjNums:
            np.random.seed(randomseed[seedcount])
            tmp_mat = fcmat[subj].copy()
            np.random.shuffle(tmp_mat)
            inputs.append((subj,tmp_mat))
            seedcount += 1
        ...
        for ruledim in ruledims:
            ...
            np.savetxt(outdir + 'NetworkITE_NullModel_' + ruledim + '_Permutation' + str(nPermutation) + '_Tstat.csv', tstats[ruledim], delimiter=',')
